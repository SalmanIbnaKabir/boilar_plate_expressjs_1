...
module.exports.test = async (req, res, next) => {
  ...
  res.send({ success: true, message: "test successfully inserted" });
};
module.exports.testGet = async (req, res, next) => {
  ...
  const result = await db
    ...
    .find({ age: 99999 })
    // .find({ _id: ObjectId("64ca0b31487464b842acccef") })
    .toArray();
  res.json(result);
  // db.test.createIndex({age: 1})  // for faster performance in mongodb
};
